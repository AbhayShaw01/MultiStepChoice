...
export default function MathList({ data }: MathListProps) {
  ...
  return (
    <div className=" h-[500px] space-y-8 sm:h-[400px]">
      ...
      <ul className="flex flex-wrap sm:flex-nowrap justify-evenly  py-2 mt-2 sm:mt-10 justify-center gap-3">
        {data.map((item, index) => (
          <li key={index} onClick={()=>showIsActive(index)} className={` ${ isActive === index
            ? "border-yellow-500 cursor-pointer border-3"
            : "hover:border-yellow-500 hover:cursor-pointer hover:border-3"} w-[200px] sm:w-full h-42 rounded-xl  text-center flex-col py-2  px-3 border hover:border-yellow-500`}>
